...
USER user={0};
uint16_t ads_ch0_global=0;
uint16_t ads_ch1_global=0;
uint16_t ads_ch2_global=0;
uint16_t ads_ch3_global=0;
...
uint16_t 	adc0_datalog,adc1_datalog,adc4_datalog,adc5_datalog;
...
{	
	...
	{
		/* model12: PA4 (ADC1) + PA0/Pin9 (ADC2) + ADS1115 4-ch I2C ADC (ADDR=0x48) */
		Sensor->adc1 = ADCModel(ADC_CHANNEL_4);
		Sensor->adc2 = ADCModel(ADC_CHANNEL_0);

		MX_I2C1_Init();
		uint16_t ads_ch0 = ADS1115_ReadChannel_mV(0);
		uint16_t ads_ch1 = ADS1115_ReadChannel_mV(1);
		uint16_t ads_ch2 = ADS1115_ReadChannel_mV(2);
		uint16_t ads_ch3 = ADS1115_ReadChannel_mV(3);
		ads_ch0_global = ads_ch0;
		ads_ch1_global = ads_ch1;
		ads_ch2_global = ads_ch2;
		ads_ch3_global = ads_ch3;
		HAL_I2C_MspDeInit(&hi2c1);
		HAL_Delay(20);

		user_main_printf("ADS1115_ch0:%u mV", ads_ch0);
		user_main_printf("ADS1115_ch1:%u mV", ads_ch1);
		user_main_printf("ADS1115_ch2:%u mV", ads_ch2);
		user_main_printf("ADS1115_ch3:%u mV", ads_ch3);

		sprintf(Sensor->data+strlen(Sensor->data), "%.4x", Sensor->adc1);
		sprintf(Sensor->data+strlen(Sensor->data), "%.4x", Sensor->adc2);
		sprintf(Sensor->data+strlen(Sensor->data), "%.4x", ads_ch0);
		sprintf(Sensor->data+strlen(Sensor->data), "%.4x", ads_ch1);
		sprintf(Sensor->data+strlen(Sensor->data), "%.4x", ads_ch2);
		sprintf(Sensor->data+strlen(Sensor->data), "%.4x", ads_ch3);
	}
...
void get_sensorvalue(void)
{
			...
			if((sys.mod!=model6)&&(sys.mod!=model9))
  {
		...
		if((sys.mod!=model3 &&sys.mod!=model11 &&sys.mod!=model12))
		{
			...
			if(sys.mod==model3 ||sys.mod==model11)
  {
	  adc1_datalog = ADCModel(ADC_CHANNEL_1);
		...
		if(sys.mod==model3)
			adc5_datalog = ADCModel(ADC_CHANNEL_5);
	}
			if(sys.mod==model4)
  {	
  ...
  {	
	  ...
	  get_tmp117_temp();
		...
		if(sys.mod==model12)
  {
				/* model12: PA0/Pin9 + ADS1115 4-channel I2C ADC */
		adc4_datalog = ADCModel(ADC_CHANNEL_0);
		MX_I2C1_Init();
		uint16_t ads_ch0 = ADS1115_ReadChannel_mV(0);
		uint16_t ads_ch1 = ADS1115_ReadChannel_mV(1);
