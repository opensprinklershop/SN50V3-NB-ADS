...
ATEerror_t at_mod_set(const char *param)
{
	...
	{
		printf("\r\nPA4 + PA0 + ADS1115 (4x I2C ADC) mode\r\n");
	}	
	else
	{
...
void config_Get(void)
{
	...
	/* Check if stored password is valid printable ASCII (handles erased 0xFF, garbage, or empty) */
	uint8_t pwd_len = strlen((char*)sys.pwd);
	uint8_t pwd_valid = (pwd_len > 0 && pwd_len <= 8);
	if(pwd_valid) {
		for(uint8_t k=0; k<pwd_len; k++) {
			if(sys.pwd[k] < 0x20 || sys.pwd[k] > 0x7E) { pwd_valid = 0; break; }
		}
	}
	if(!pwd_valid)
	{
		memcpy(sys.pwd,"377612",7);
	  password_parameters[0]=sys.pwd[0]<<24 | sys.pwd[1]<<16 	| sys.pwd[2]<<8 | sys.pwd[3];
	  password_parameters[1]=sys.pwd[4]<<24 | sys.pwd[5]<<16 	| sys.pwd[6]<<8 | sys.pwd[7];	
