...
//	return adc_mV_;
	...
	if(channel == ADC_CHANNEL_4)
		...
		user_main_printf("adc_0.1mV/V(4):%.2f",adc_mV_average);
  }
 }
	else
	{
		uint16_t measuredLevel = HW_AdcReadChannel( channel );
		uint16_t adc_mV = (uint16_t)((uint32_t)measuredLevel * sensor.batteryLevel_mV / 4095);
 		if(tdc_clock_log_flag==0)
	{
	if(channel == ADC_CHANNEL_4)
		user_main_printf("adc_mV(PA4/ADC1):%u mV",adc_mV);
	else if(channel == ADC_CHANNEL_5)
		user_main_printf("adc_mV(PA5/ADC2):%u mV",adc_mV);
	else if(channel == ADC_CHANNEL_0)
		user_main_printf("adc_mV(PA0=Pin9/ADC3):%u mV",adc_mV);
	else if(channel == ADC_CHANNEL_5)
		user_main_printf("adc_mV(PA5):%u mV",adc_mV);
